---
title: Costa Rica
description: Nodo nacional
language: es
language_reference: member-costa-rica
menu_parent: members
menu_order: 70
menu_label: Costa Rica
menu_icon: org
summary: Costa Rica ingresó a LA Referencia en el año 2016.
menu_summary: Repositorio Nacional de Costa Rica (KIMUK)
node_name: Repositorio Nacional de Costa Rica (KIMUK)
country_code: CR
legacy_url: https://dev.lareferencia.info/nodos/costa-rica/
published: true
flag_image: /assets/img/members/costa-rica-flag.png
hero_image: /assets/img/members/costa-rica-1.png
member_tabs:
- title: Consejo Directivo
  content_html: '<ul>

    <li><strong>Institución Política:</strong> Consejo Nacional de Rectores (<a href="https://www.conare.ac.cr/" rel="noopener" target="_blank">CONARE</a>)</li>

    <li><strong>Consejo Directivo:</strong> Carolina Seas y Sonia Artavia Cruz (CONARE, Costa Rica)</li>

    <li><strong>Comité Técnico:</strong> Guillermo Murillo Goussen (UCR-CONARE, Costa Rica)</li>

    </ul>'
- title: Legislación y Políticas
  content_html: ''
- title: Directrices y Reglamentos
  content_html: <p><a href="http://kimuk.conare.ac.cr/Contents/Home?section=who">Políticas de aceptación para nuevos miembros</a></p>
- title: Enlaces de interés
  content_html: <ul><li><a href="http://kimuk.conare.ac.cr/" rel="noopener" target="_blank">Kimuk<br/></a></li></ul>
node_heading: Nodo Costa Rica
node_subheading: Repositorio Nacional Kímuk
node_image: /assets/img/members/costa-rica-2.png
node_copy: El repositorio Nacional Kímuk es una herramienta de acceso abierto impulsada por el Consejo Nacional de Rectores (CONARE), cuyo objetivo es ofrecer acceso en línea a la producción académica y científica nacional, como un insumo crucial para el desarrollo científico y tecnológico del país y por ello su visibilidad y acceso debe ser prioritario.
stats_heading: Estadísticas del Nodo Costa Rica
stats_cards:
- value: '41'
  title: Instituciones Cosechadas
  link_text: Conocer el listado completo de las instituciones >
- value: '280.971'
  title: Recursos
  link_text: Conocer el listado completo de los recursos >
- value: '280.971'
  title: Recursos
  link_text: Conocer el listado completo de las instituciones >
contact_question: ¿Su institución se encuentra en Costa Rica y desea ser parte del nodo?
contact_prompt: Póngase en contacto con las autoridades costarricenses para integrarse
contact_cards:
- title: Teléfono
  description: 'Tel: (+506) 2511-4412'
- title: Email
  description: kimuk@conare.ac.cr
  href: mailto:kimuk@conare.ac.cr?subject=Información%20para%20integrarse%20al%20Nodo
- title: Dirección
  description: Subcomisión de Conocimiento Abierto de CONARE, San José
---

